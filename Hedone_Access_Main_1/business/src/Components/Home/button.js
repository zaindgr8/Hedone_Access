import React, { useEffect } from 'react';

function Button1({title}) {
  useEffect(() => {
    if (typeof window !== "undefined") {
      const btn = document.querySelector(".btn-1");

      btn.onmousemove = function (e) {
        const x = e.pageX - btn.offsetLeft;
        const y = e.pageY - btn.offsetTop;

        btn.style.setProperty("--x", x + "px");
        btn.style.setProperty("--y", y + "px");
      };
    }
  }, []);

  return (
    <div className='btn-1 relative overflow-hidden inline-flex px-3 py-1 bg-gray-200 
     text-xl tracking-widest font-Cinzel
    before:absolute before:w-0
    before:top-[var(--y)] before:left-[var(--x)] before:h-0
    before:rounded-full before:bg-white text-white border-white
    hover:before:w-[400px] hover:before:h-[400px] hover:text-black
    before:translate-x-[-50%] before:translate-y-[-50%] hover:before:scale-110
    rounded-3xl bg-opacity-10 border-2'>
     <span className="inline-block z-10 relative">{title}</span>
    </div>
  );
}

export default Button1;




// import React, { useEffect } from 'react';

// function Button({title}) {
//   useEffect(() => {
//     if (typeof window !== "undefined") {
//       const btn = document.querySelector(".btn");

//       btn.onmousemove = function (e) {
//         const x = e.pageX - btn.offsetLeft;
//         const y = e.pageY - btn.offsetTop;

//         btn.style.setProperty("--x", x + "px");
//         btn.style.setProperty("--y", y + "px");
//       };
//     }
//   }, []);

//   return (
//     <div className='btn mt-5 relative overflow-hidden inline-flex px-8 py-3 bg-gray-200 
//      text-xl tracking-widest 
//     before:absolute before:w-0
//     before:top-[var(--y)] before:left-[var(--x)] before:h-0
//     before:rounded-full before:bg-white text-white
//     hover:before:w-[400px] hover:before:h-[400px]
//     before:translate-x-[-50%] before:translate-y-[-50%] hover:before:scale-110
//     rounded-3xl bg-opacity-10 border-2 hover:text-black'>
//      <span className="inline-block z-10 relative">{title}</span>
//     </div>
//   );
// }

// export default Button;
